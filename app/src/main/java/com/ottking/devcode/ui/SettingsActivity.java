...
public class SettingsActivity extends AppCompatActivity {
    ...
    private void setupPlayerView(View view) {
        ...
        // 5. HLS Mode Setup
        Spinner spinnerHlsMode = view.findViewById(R.id.spinnerHlsMode);
        ...
        if (spinnerHlsMode != null) {
            spinnerHlsMode.setAdapter(hlsAdapter);
            String curHls = prefs.getHlsSettings();
            int curHlsIdx = 0;
            for (int i = 0; i < hlsModes.length; i++) {
                if (hlsModes[i].equalsIgnoreCase(curHls)) { curHlsIdx = i; break; }
            }
            spinnerHlsMode.setSelection(curHlsIdx);
            spinnerHlsMode.setOnItemSelectedListener(new android.widget.AdapterView.OnItemSelectedListener() {
                @Override
                public void onItemSelected(android.widget.AdapterView<?> parent, View v, int position, long id) {
                    if (v instanceof TextView) ((TextView) v).setTextColor(getColor(R.color.text_primary));
                    prefs.setHlsSettings(hlsModes[position]);
                }
                @Override public void onNothingSelected(android.widget.AdapterView<?> parent) {}
            });
        }
        ...
        // 6. MPD Mode Setup
        Spinner spinnerMpdMode = view.findViewById(R.id.spinnerMpdMode);
        ...
        if (spinnerMpdMode != null) {
            spinnerMpdMode.setAdapter(mpdAdapter);
            String curMpd = prefs.getMpdSettings();
            int curMpdIdx = 0;
            for (int i = 0; i < mpdModes.length; i++) {
                if (mpdModes[i].equalsIgnoreCase(curMpd)) { curMpdIdx = i; break; }
            }
            spinnerMpdMode.setSelection(curMpdIdx);
            spinnerMpdMode.setOnItemSelectedListener(new android.widget.AdapterView.OnItemSelectedListener() {
                @Override
                public void onItemSelected(android.widget.AdapterView<?> parent, View v, int position, long id) {
                    if (v instanceof TextView) ((TextView) v).setTextColor(getColor(R.color.text_primary));
                    prefs.setMpdSettings(mpdModes[position]);
                }
                @Override public void onNothingSelected(android.widget.AdapterView<?> parent) {}
            });
        }
