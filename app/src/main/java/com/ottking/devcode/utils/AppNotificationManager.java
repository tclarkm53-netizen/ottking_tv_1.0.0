...
public class AppNotificationManager {
    ...
    public void loadNotifications(OnNotificationsUpdatedListener listener) {
        new Thread(() -> {
            ...
            // 3. Fetch server notifications from /notifications route
            ApiClient.getInstance(context).fetchNotifications(new ApiClient.ApiCallback<List<NotificationItem>>() {
                @Override
                public void onSuccess(List<NotificationItem> serverNotifs) {
                    if (serverNotifs != null && !serverNotifs.isEmpty()) {
                        ...
                        for (NotificationItem sItem : serverNotifs) {
                            if (!readIds.contains(sItem.getId())) {
                                list.add(0, sItem);
                                added = true;
                            }
                        }
                        if (added && listener != null) {
                            int totalUnread = list.size();
                            new android.os.Handler(android.os.Looper.getMainLooper()).post(() ->
                                    listener.onNotificationsUpdated(new ArrayList<>(list), totalUnread));
                        }
                    }
                }
